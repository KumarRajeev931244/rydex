...
function RideChat({currentRole,bookingId,userName,driverName}:any) {
    ...
    const [suggesstion, setSuggesstion] = useState<string[]>([]);
  ...
  return () => {
    ...
    const getAISuggesstion=async()=> {
        ...
        setShowAI(true)
        try {
            const {data} = await axios.post("/api/chat/ai-suggesstion",{
               role:currentRole,
               lastMessages
            })
            console.log("getAISuggesstion:",data);
            const jsonData = JSON.parse(data)
            setSuggesstion(jsonData.suggestions)
            setAiLoading(false)
        } catch (error) {
            console.log(error);
            setAiLoading(false)
    ...
    // const get = async()=> {
    //     try {
    //         const data = await axios.get("/api/chat/ai-suggesstion")
    //         console.log(data);
    //     } catch (error) {
    //         console.log(error);
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-zinc-50" style={{scrollbarWidth:"none", msOverflowStyle:"none"}}>
            ...
            {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full gap-3 py-16">
                    ...
                    <p className="text-sm text-zinc-400 font-medium">No message yet</p>
                        ...
                        <div className="px-4 pt-3 pb-2">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">AI Suggesstion</span>
                                </div>
                                <button onClick={() => setShowAI(false)}>
                                    <X size={14} className="text-zinc-400 hvoer:text-zinc-600"/>
                                </button>
                            </div>
            ...
            <div className="flex items-center gap-2 bg-zinc-100 rounded-2xl pl-3 py-1.5">
                {messages.length > 0 && (
                    ...
                    className={`flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all ${
                        showAI ? "bg-violet-600 text-white" : "bg-white text-violet-500 hover:bg-violet-50 border border-zinc-200"
                    }`}
                    >
                        <Sparkles size={14}/>
                    </motion.button>
                ...
                onClick={()=>sendMessage()}
                className={`flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all ${
                    text.trim() ? "bg-zinc-950 text-white hover:bg-zinc-800" : "bg-transparent text-zinc-300 cursor-not-allowed"
                }`}
                >
                    <Send size={14}/>
                </motion.button>

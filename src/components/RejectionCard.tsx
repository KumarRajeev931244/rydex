...
export default function RejectionCard({title,reason,actionLabel,onAction}:any){
    return(
        <div className="bg-red-50 border border-red-200 rounded-2xl md:rounded-3xl p-5 sm:p-6 md:p-8 space-y-4">
            ...
            {onAction &&  (
                <button className="sm:w-auto px-6 py-2.5 bg-black text-white rounded-xl text-sm sm:text-base font-medium hover:bg-gray-800 transition" onClick={onAction}>{actionLabel || "retry"} </button>
            )}
        </div>
    )
}
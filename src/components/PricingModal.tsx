...
export default function PricingModal({open,onClose,data}:PropsType){
    ...
    return(
        <AnimatePresence>
            {open && (
                ...
                animate={{opacity:1}}
                exit={{opacity:0}}
                className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 px-4"
                >
                    <motion.div
                    initial={{scale:0.85}}
                    animate={{scale:1}}
                    className='w-full bg-white  max-w-lg rounded-3xl shadow-2xl overflow-hidden mt-20'
                    ...
                    >
                        ...
                        <label 
                        htmlFor='imageLabel' 
                        className="relative h-44 border-2 border-dashed rounded-2xl flex items-center justify-center cursor-pointer w-full overflow-hidden"
